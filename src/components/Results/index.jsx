...
const Results = ({ results, current }) => {
	...
	return (
		<section className="result mx-auto max-w-xl md:max-w-2xl lg:max-w-3xl xl:max-w-4xl">
			<h2 className="text-xl md:text-2xl lg:text-3xl xl:text-4xl mb-6">Pics to words: лучшие результаты</h2>
			<table className="result-table w-full mb-8">
				...
				<tbody>
					{sortResults.map(({ name, steps }, index) => (
						<tr key={name} className={steps === current ? 'result-table-row active' : 'result-table-row'}>
			...
			<button onClick={handleResetGame} className="button px-4 py-2">Новая игра</button>
		</section>
	);
}
